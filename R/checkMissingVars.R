...
#' @param source source than can be passed to getMappingVariables
#' @importFrom piamutils deletePlus
#' @importFrom quitte as.quitte
#' @return an invisible vector with missing variables
#' @export
checkMissingVars <- function(mifdata, mapping, source = TRUE) {
  mifdata <- as.quitte(mifdata)
  mappingVariables <- deletePlus(getMappingVariables(mapping, source))
  ...
  if (length(missingVariables) > 0) {
    message("# The following ", length(missingVariables), " variables are expected in the piamInterfaces package ",
            "for mapping ", paste(mapping, collapse = ", "), " and source ", source,
